...
- 208 missions and 555 conversations active; 299 missions remain inactive stubs because their destination, availability, or control script has no safe ES equivalent yet. EVN `AvailBits` boolean expressions now map through ES `and`/`or` conditions, including `P0` registration tests and recoverable `O` outfit tests. Government, ally, enemy, class, non-class, and adjacent-system stellar selectors map through EVN government relations and ES location filters. No-specific-return missions omit the ES destination filter; exact stellar travel values become system waypoints when recoverable. Mission-computer offers map to `job`, outfit-dialog offers map to `outfitter`, exact EVN ship-type gates map to ES flagship-model conditions, and statically resolvable legal-record gates map to ES reputation thresholds with explicit approximation comments.
...
3. Complete mission conversion: map remaining generic stellar selectors, dynamic initial-stellar returns, domination/record rules, and `S`/`A`/`M`/`N`/`P`/`T`/`C`/`E`/`H`/`Y`/`U`/`Q` action opcodes where ES gains a tested equivalent, and replace approximation comments with engine behavior.
