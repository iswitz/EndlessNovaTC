...
- EVN `crön` `STR#` news now emits 97 ES `news` definitions. Start events activate independent news near Sol as an ES approximation of EVN global news and activate local news by mapped government; end events remove those locations.
...
2. Add ES date-window mappings for the remaining `crön` records; replace landing-time approximations with a lower-overhead scheduler where possible.
