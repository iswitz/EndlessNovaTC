...
- EVN ship `MaxGun` and `MaxTur` limits now drive ES weapon slots. EVN gun, guided, turret, and fighter exit points now emit ES hardpoints; fighter payload names remain comments because ES bay lines use `Fighter` / `Drone` categories.
...
- EVN fixed-gun, turret, and carried-ship weapon flags now map to mountable ES gun or turret outfits; unsupported EVN weapon slot flags remain generated comments.
- EVN `WeapType` / `WeapCount` / `AmmoLoad` fields now rebuild stock ship loadouts with exact weapon counts and ES ammunition outfits. Shared EVN ammo types resolve across launcher and turret variants; carried-ship payload counts remain explicit comments because ES fighter bays use category mounts.
- EVN dude AI types now map to ES fleet personalities: wimpy traders become timid/coward, brave traders uninterested, warships heroic, and interceptors heroic/surveillance. Ship swarming, ammo-frugality, asteroid-mining, and debris-harvesting flags map where ES has close personality equivalents; unsupported cloak, standoff, blind-spot, and escort-only behavior remains commented.
- EVN `bööm` explosion animations now emit 15 ES `effect` definitions with local EVN sound links. Converted ships emit matching `explode` and `final explode` entries; EVN `DeathDelay` remains an explicit timing approximation comment.
...
- 256 missions and 555 conversations active; 251 missions remain inactive stubs because their destination, availability, or control script has no safe ES equivalent yet. EVN `AvailBits` boolean expressions now map through ES `and`/`or` conditions, including `P0` registration tests and recoverable `O` outfit tests. Government, ally, enemy, class, non-class, and adjacent-system stellar selectors map through EVN government relations and ES location filters. No-specific-return missions omit the ES destination filter; exact stellar travel values become system waypoints when recoverable. Mission-computer offers map to `job`, outfit-dialog offers map to `outfitter`, exact EVN ship-type gates map to ES flagship-model conditions, statically resolvable legal-record gates map to ES reputation thresholds, negative EVN clean-record payments map to reputation resets with explicit approximation comments, and active EVN `Axxx` abort actions map to ES `fail` actions with semantic-difference comments.
...
3. Complete mission conversion: map remaining generic stellar selectors, dynamic initial-stellar returns, domination/record rules, negative credit penalties, and `S`/`A`/`M`/`N`/`P`/`T`/`C`/`E`/`H`/`Y`/`U`/`Q` action opcodes where ES gains a tested equivalent, and replace approximation comments with engine behavior.
4. Improve ship and weapon fidelity: remaining AI behavior, visual scaling, explosion timing, and parser warnings.
...
Validate generated `crön` and `öops` scheduler missions:

```text
npm run validate:schedulers
```
...
Latest startup test also covered rebuilt stock weapon and ammunition loadouts; no new log entries appeared.

Generated fleet definitions now carry EVN AI personality mappings and selected ship behavior flags.

EVN explosion effects now load through ES `effect` records with 15 local sprite aliases and EVN sound links. Converted ships use those effects for initial and final destruction stages. The latest startup test produced no new effect or sound parser warnings; existing shuttle frame-size warnings remain.

Scheduler smoke test loaded 124 `crön` schedulers, 19 `öops` schedulers, 256 cron events, and 38 disaster events without new parser errors. Landing-triggered price changes and delayed reset timing still need live save-game verification.

`validate:schedulers` checks scheduler mission structure and dangling event references. It passes with 124 `crön` missions, 19 `öops` missions, and zero missing event references.

Endless Sky loaded the plugin without new log entries after emitting EVN hardpoints, correcting ES gun/turret outfit slots, and mapping `MaxGun` / `MaxTur`. Ship thumbnail aliases and planet landscape aliases are also generated. Planet records now recover and emit EVN government assignments for 306 planets. EVN dock flags now drive ES landing, outfitter, and shipyard entries. Thirty-five generic tech-tier outfitters and 20 generic tech-tier shipyards are generated. Planet `SpecialTech` values now create 154 dedicated outfitters and 49 dedicated shipyards with exact-tech EVN items. Tech-tier license outfits now gate converted ships and outfits. System records now emit 542 ES trade prices from EVN commodity exchange flags, EVN controlling governments, asteroid fields, 2,708 traffic/reinforcement fleet schedules, and 275 fleet definitions from `sÿst`, `spöb`, `düde`, and `flët` metadata; EVN murk now selects ES system haze artwork on 187 systems, while sensor interference and visibility expressions remain explicit EVN system attributes. Planet records now emit 259 ES tribute entries and 216 defense-fleet mappings; EVN encoded defense waves become total ES tribute-fleet spawns. EVN `nëbu` map decorations remain in `source/nëbu.json` because ES has no direct nebula-region syntax. `düde` probability lists become weighted ES fleet variants; `flët` lead/escort formations become fixed variants using mean escort counts. Mission conversion now emits 256 active missions, 251 inactive stubs, and 682 conversations, including complex availability filters, relation-aware stellar selectors, no-specific-return missions, system waypoints, location types, ship-model gates, legal-record approximations, clean-record reputation resets, mapped `Axxx` abort actions, rating gates, cargo/person text, failure/abort paths, control-bit transitions, reputation rewards, real EVN text branch trees, verified outfit/rank/exploration/message actions, and explicit comments for unsafe opcodes. The EVN `.Trader` `chär` template now appears as an `Endless Nova Trader` ES start at Kania / Port Kane with 25,000 credits, a Shuttle, a pilot license, and EVN registration. Plugin folder is now `endless-nova-tc`, with metadata name `Endless Nova TC`; its menu background now overwrites the stock interface with a unique EVN/ES hybrid sprite, `ENDLESS NOVA TC` lettering, and an Endless Sky star accent. The installed copy now uses EVN's 23/6/1177 NC start date, loads 124 converted `crön` events and 99 `STR#` news definitions, and the latest startup test produced no new parser or attribute warnings.
